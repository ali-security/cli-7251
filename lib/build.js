...
function linkBins (pkg, folder, parent, gtop, cb) {
  ...
  asyncMap(Object.keys(pkg.bin), function (b, cb) {
    // Normalize the bin path to prevent directory traversal
    var cleanB = path.join("/", b).replace(/\\|:/g, "/").substr(1)
    var binPath = pkg.bin[b]
    if (typeof binPath !== "string") {
      return cb()
    }
    var cleanBinPath = path.join("/", binPath).replace(/\\|:/g, "/").substr(1)

    linkBin( path.resolve(folder, cleanBinPath)
           , path.resolve(binRoot, cleanB)
           , gtop && folder
           , function (er) {
      if (er) return cb(er)
      // bins should always be executable.
      // XXX skip chmod on windows?
      var src = path.resolve(folder, cleanBinPath)
      fs.chmod(src, npm.modes.exec, function (er) {
        if (er && er.code === "ENOENT" && npm.config.get("ignore-scripts")) {
          return cb()
        }
        if (er || !gtop) return cb(er)
        var dest = path.resolve(binRoot, cleanB)
...
function linkMans (pkg, folder, parent, gtop, cb) {
  ...
  var set = pkg.man.reduce(function (acc, man) {
    ...
    acc[path.basename(cleanMan)] = cleanMan
    return acc
  }, {})
  pkg.man = pkg.man.filter(function (man) {
    ...
    var cleanMan = path.join("/", man).replace(/\\|:/g, "/").substr(1)
    return set[path.basename(cleanMan)] === cleanMan
  })

  asyncMap(pkg.man, function (man, cb) {
    if (typeof man !== "string") return cb()
    var cleanMan = path.join("/", man).replace(/\\|:/g, "/").substr(1)
    log.silly("linkMans", "preparing to link", cleanMan)
    var parseMan = cleanMan.match(/(.*\.([0-9]+)(\.gz)?)$/)
    if (!parseMan) {
      return cb(new Error(
        cleanMan+" is not a valid name for a man file.  " +
    ...
    var manSrc = path.resolve(folder, cleanMan)
